...
stock_const_func <- function(choice, int_state, a, b, p, gamma, alpha, discount, final_state) {
  ...
  state <- numeric(length = 10)
  const <- numeric(length = 10)
  
  #state <- vector(mode="numeric", length=0)
  #const <- vector(mode="numeric", length=0)
  
  # Set final state and constraint for t=10
...
```{r}
stock_2a_table<-function(choice, int_state=40, period=10, a=2, b=-0.25, p=100, alpha=2, gamma=0.2, discount=1/1.04, final_state=25){
