...
stock_obj_func <- function(choice, z0, a, b, p, gamma, alpha, rho) {
  ...
  state[1] = z0
  t=seq(0, period)
  benefits[1] = rho^t*(p*choice[1]- (a*state[1]-b*state[1]^2))
  
  for(i in 2:period) {
    state[i] = state[i-1]*(1-gamma)+alpha*choice[i-1]
    benefits[i] = rho^t*(p*choice[i]-(a*state[i]-b*state[i]^2))
  }
...
stock_const_func <- function(choice, z0, a, b, p, gamma, alpha, rho) {
...
```{r}
stock_2a_table<-function(choice, z0=40, period=10, a=2, b=-0.25, p=100, alpha=2, gamma=0.2, rho=1/1.04){
  ...
  return(data.frame("period"=t, "npv"=npv, "stock"=state, "effort"=choice))
